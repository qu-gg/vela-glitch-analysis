...
def get_window(signals):
    ...
    if center - 100 < 0:
        offset = abs(center - 100)
        center += offset
        signals = np.pad(signals, ((0, 0), (offset, 0)), mode='median') #, constant_values=(0, 0))

    print("Window: ({}, {})".format(center - 100, center + 100))
    return signals[:, center - 100:center + 100]


# Load in pre-glich days
d1 = np.load("data/july19/rfiBoth/A1.npy", allow_pickle=True)
d2 = np.load("data/july20/rfiBoth/A1.npy", allow_pickle=True)
d3 = np.load("data/july21/rfiBoth/A1.npy", allow_pickle=True)
dataset = np.vstack((d1, d2, d3))
...
if args.resume is True:
    model.load_state_dict(torch.load('models/preglitch{}_vae.torch'.format(antenna)))
...
for epoch in range(args.epochs):
    ...
    torch.save(model.state_dict(), 'models/preglitch{}_vae.torch'.format(antenna))
...
torch.save(model.state_dict(), 'models/preglitch{}_vae.torch'.format(antenna))
print("=> Model saved at ", 'models/preglitch{}_vae.torch'.format(antenna))
...
np.savetxt('reconstructed/preglitch{}recon_vae.csv'.format(antenna), recons, delimiter=',')
